...
| `diagnostician_done` | critic | the diagnostician wrote `GATE<N>-HINT.md` — `bytes`, and `case`: its declared first line, `grounding` \| `real_gap` \| `unknown`. Read-only metric; `learn.py summarize` counts it per phase, the gate never reads it |
