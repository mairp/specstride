...
- **Grounded critic.** Before the LLM call, the critic verifies the files the evidence cites
  ...
  `SPECSTRIDE_CRITIC_GROUNDING` (on by default).
...
- **Single-run lock, timeouts, wall budget, `stop.flag`.** One orchestrator per workdir; a
  per-pass timeout (`SPECSTRIDE_PROPOSER_TIMEOUT`) and a per-critic-call timeout
  (`SPECSTRIDE_CRITIC_TIMEOUT`); an optional whole-run wall-clock budget (`SPECSTRIDE_MAX_WALL_MIN`); a
  manual clean halt via `specstride stop`.
...
- **Per-phase git checkpoint.** After each `GATE<N>-APPROVED`, if the workdir is a git repo with
  changes, the orchestrator commits `specstride: phase <N> approved — <title>`. **Never inits, never
  pushes.** Governed by `SPECSTRIDE_GIT_COMMITS` (auto).
...
| `6` | stopped via `stop.flag` (clean; `specstride resume` or rerun continues). Also produced when the stop lands **mid-proposer** (`specstride stop --now`) — earlier versions mislabeled this as `4` |
