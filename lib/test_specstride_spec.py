#!/usr/bin/env python3
"""Tests for lib/specstride_spec.py — the single source of truth for spec parsing.
...
bash↔python parity check (the specstride-lib.sh shim output equals the Python CLI for
...
def test_native_detect():
    assert specstride_spec.detect_format(NATIVE_SPEC, read(NATIVE_SPEC)) == "native"


def test_native_phases_and_titles():
    phases = specstride_spec.get_phases(read(NATIVE_SPEC), "native")
...
def test_native_validate_ok():
    ok, count, errors = specstride_spec.validate(read(NATIVE_SPEC), "native")
    assert ok and count == 2 and errors == []


def test_native_slice_matches_heading():
    section = specstride_spec.slice_phase(read(NATIVE_SPEC), 1, "native")
...
def test_native_validate_errors(text, expected_err):
    ok, _count, errors = specstride_spec.validate(text, "native")
    assert not ok
...
def test_native_start_at_one_is_valid():
    ...
    text = ("## Phase 1 — x\n### Acceptance criteria\n- [ ] a\n"
            "## Phase 2 — y\n### Acceptance criteria\n- [ ] b\n")
    ok, count, _ = specstride_spec.validate(text, "native")
...
def test_native_phase_heading_case_sensitive():
    ...
    text = "## phase 0 — x\n### Acceptance criteria\n- [ ] a\n"
    assert specstride_spec.get_phases(text, "native") == [] or \
        [p.n for p in specstride_spec.get_phases(text, "native")] == []
...
def test_speckit_detect_by_filename():
    ...
    assert specstride_spec.detect_format("/x/tasks.md", "## Phase 1: Setup\n- [ ] T001 do\n") \
        == "speckit-tasks"
...
def test_speckit_detect_by_content():
    ...
    assert specstride_spec.detect_format("/x/FEATURE.md", text) == "speckit-tasks"


def test_speckit_phases():
    phases = specstride_spec.get_phases(read(SPECKIT_SPEC), "speckit-tasks")
...
def test_speckit_validate_ok():
    ok, count, errors = specstride_spec.validate(read(SPECKIT_SPEC), "speckit-tasks")
    assert ok and count == 3 and errors == []


def test_speckit_phase_without_tasks_rejected():
    text = "## Phase 1: Setup\nprose only, no checkboxes\n"
    ok, _c, errors = specstride_spec.validate(text, "speckit-tasks")
    assert not ok
...
def test_speckit_priority_groups_become_unique_ordered_phases():
    phases = specstride_spec.get_phases(PRIORITY_TASKS, "speckit-tasks")
...
def test_speckit_priority_groups_validate_with_repeated_priorities():
    ok, count, errors = specstride_spec.validate(PRIORITY_TASKS, "speckit-tasks")
    assert ok and count == 4 and errors == []


def test_speckit_priority_groups_detect_by_content():
    assert specstride_spec.detect_format("/x/work-items.md", PRIORITY_TASKS) \
        == "speckit-tasks"
...
def test_openspec_detect_by_canonical_path():
    ...
    assert specstride_spec.detect_format(path, text) == "openspec-change"


def test_openspec_detect_by_content():
    assert specstride_spec.detect_format("/x/work-items.md", read(OPENSPEC_SPEC)) \
        == "openspec-change"


def test_openspec_phases_and_tasks():
    phases = specstride_spec.get_phases(read(OPENSPEC_SPEC), "openspec-change")
    assert [p.n for p in phases] == [1, 2, 3]
    ...
    assert specstride_spec.phase_title(phases[1].section) == "Implementation"


def test_openspec_validate_rejects_taskless_section():
    text = "## 1. Setup\nprose only\n"
    ok, count, errors = specstride_spec.validate(text, "openspec-change")
...
def test_openspec_context_and_order(tmp_path):
    ...
    ctx = specstride_spec.openspec_context(str(tasks))
    assert list(ctx) == [
        "proposal",
    ...
    rendered = specstride_spec.render_context(
...
def test_openspec_feature_slug_is_change_name(tmp_path):
    tasks = _make_openspec_change(tmp_path)
    assert specstride_spec.feature_slug(str(tasks)) == "add-audit-export"
...
def test_explicit_override_beats_sniff():
    ...
    assert specstride_spec.detect_format("/x/tasks.md", text, override="native") == "native"


def test_env_override(monkeypatch):
    monkeypatch.setenv("SPECSTRIDE_SPEC_FORMAT", "native")
    assert specstride_spec.detect_format("/x/tasks.md", "## Phase 1: S\n- [ ] T1 x\n") == "native"


def test_unknown_format_raises():
    with pytest.raises(ValueError):
        specstride_spec.detect_format("/x/y.md", "text", override="bogus")
...
def test_speckit_context(tmp_path):
    ...
    ctx = specstride_spec.speckit_context(str(tasks))
...
def test_context_empty_outside_specify(tmp_path):
    ...
    assert specstride_spec.speckit_context(str(f)) == {}
...
def test_context_order_by_gating_value(tmp_path):
    tasks = _make_feature(tmp_path)
    keys = list(specstride_spec.speckit_context(str(tasks)).keys())
...
def test_feature_slug_under_specify(tmp_path):
    tasks = _make_feature(tmp_path)
    assert specstride_spec.feature_slug(str(tasks)) == "001-demo"


def test_feature_slug_default_for_native(tmp_path):
    f = tmp_path / "SPECS.md"
    f.write_text("## Phase 0 — x\n### Acceptance criteria\n- [ ] a\n")
    assert specstride_spec.feature_slug(str(f)) == "default"
...
def test_feature_slug_sanitized(tmp_path):
    ...
    tasks = feature / "tasks.md"
    tasks.write_text("## Phase 1: S\n- [ ] T001 do\n")
    slug = specstride_spec.feature_slug(str(tasks))
...
def test_render_context_empty_for_native(tmp_path):
    f = tmp_path / "SPECS.md"
    f.write_text("## Phase 0 — x\n### Acceptance criteria\n- [ ] a\n")
    assert specstride_spec.render_context(str(f)) == ""
...
def test_render_context_line_clean_and_fence_safe(tmp_path):
    ...
    out = specstride_spec.render_context(str(tasks), budget=6000)
...
def test_render_context_budget_respected_and_floors(tmp_path):
    ...
    out = specstride_spec.render_context(str(tasks), budget=8000)
...
def test_allocate_budget_floor_drops_slivers():
    ...
    allocs = specstride_spec._allocate_budget(sizes, 2000, 1200)
...
def _cli(*args):
    return subprocess.run([sys.executable, os.path.join(HERE, "specstride_spec.py"), *args],
                          capture_output=True, text=True)


def _shim(func, *args):
    """Call a specstride_spec_* function through specstride-lib.sh (the bash side)."""
    script = '. "%s"; %s %s' % (os.path.join(ROOT, "specstride-lib.sh"), func,
                                " ".join('"%s"' % a for a in args))
    return subprocess.run(["bash", "-c", script], capture_output=True, text=True)


def test_bash_shim_numbers_match_python():
    shim = _shim("specstride_spec_phase_numbers", NATIVE_SPEC)
    cli = _cli("numbers", "--specs", NATIVE_SPEC)
    assert shim.stdout == cli.stdout == "0\n1\n"


def test_bash_shim_validate_matches_python():
    shim = _shim("specstride_spec_validate", NATIVE_SPEC)
...
def test_bash_shim_slice_matches_python():
    shim = _shim("specstride_spec_slice", NATIVE_SPEC, "0")
...
def test_first_unapproved_explicit_gates_dir(tmp_path):
    ...
    # NOT derive <workdir>/.specstride/gates.
