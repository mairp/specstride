# Example spec — a tiny, runnable Specstride project
...
./orchestrator.sh -w /tmp/specstride-demo
```
...
exactly the single line `Hello, Specstride!` (with a trailing newline).

### Acceptance criteria
- [ ] A file `hello.txt` exists in the working directory.
- [ ] Its contents are exactly `Hello, Specstride!` followed by a newline — no other
...
a `name` key set to the string `specstride-demo` and a `phase` key set to the number
`1`.
...
- [ ] It has a top-level key `name` equal to `"specstride-demo"` and a top-level key
